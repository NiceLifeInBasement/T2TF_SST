...
def common_history(tracks):
    """
    PLEASE NOTE THAT THIS HAS PERFORMANCE ISSUES
    INSTEAD, ITS SUGGESTED THAT YOU USE THE COMMON_HISTORY_SORTED FUNCTION, AFTER SORTING THE INPUT TRACKS
        for sorting, you can use sorted(track, key=acq_timestamp)
    ...
    This version is using the closest_match function and overall ends up being O(n^2), since it is a basic
    implementation of the naive approach of matching the two lists against each other.
    ...
    for entry in tracks[smallest_track]:
        ...
        for track in tracks:
            ...
            match = closest_match(track, timestamp)  # Iterates once over track
...
def sec_diff(a, b):
    ...
    comparison. Most notably, this returns absolute values instead of negative values if b is before a
    :param a: rospy.Time object one
    :param b: rospy.Time object two
    :return: Float diff between the two times, in abs
    """
    d = b - a
    d.secs = abs(d.secs)
    d.nsecs = abs(d.nsecs)
...
def common_history_sorted(tracks):
    """
    This function is a different implementation of the common_history function. Please also refer to that function.
    ...
    This means performance can be significantly improved.
    :param tracks: A list of tracks, which are lists of TrackedOrientedBoxes. Can be produced by
        TrackingHistory.get_timed. These explicitly don't need to have the same length.
    :return: A list of tracks, which are lists of TrackedOrientedBoxes. These tracks will have the same length, and
    their content will be chosen based on time stamps. The new length will be the maximum possible length, which is the
...
def t2t_distance_historic(track_a, track_b, state_space=(True, False, False, True), use_identity=False):
    ...
    try:
        ...
        avg_len = []
        avg_common = []
    avg_timed.append(time.clock()-timed)  # Time elapsed in the whole function
    avg_len.append(len(track_a))  # Length of the track history that was used
    avg_common.append(timed_common-timed)  # Time elapsed until a common history between the tracks was built
    percent_common = avg_common[-1] / avg_timed[-1] * 100  # above, but in %
    # Now print all this information
    print(str(sum(avg_timed)/len(avg_timed))+" avg seconds to process after nsteps="+str(len(avg_timed)))
    print("\tAverage Length of the tracks: "+str(sum(avg_len)/len(avg_len)))
    print("\tAverage Time spent creating a common history: " + str(sum(avg_common) / len(avg_common)))
    print("\tThis is a percent value of %: "+str(percent_common))
    print("")
